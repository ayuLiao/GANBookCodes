...
class infoGAN(object):
    ...
    def __init__(self, sess, epoch, batch_size, z_dim, dataset_name, checkpoint_dir, result_dir, log_dir, SUPERVISED=True):
        ...
        # code
        self.len_discrete_code = 10  # categorical distribution (i.e. label) 分类分布
        self.len_continuous_code = 2  # gaussian distribution (e.g. rotation, thickness) 高斯分布

        # load fashionmnist
        fashionmnist = input_data.read_data_sets(r'/Users/ayuliao/Desktop/GAN/data/fashion-mnist/data/fashion',
                                                 one_hot=True)
        train_img = fashionmnist.train.images
        self.data_X = train_img.reshape(len(train_img), 28, 28, 1)
        self.data_y = fashionmnist.train.labels #fashionmnist标签
    ...
    def classifier(self, x, is_training=True, reuse=False):
        ...
        # All layers except the last two layers are shared by discriminator 除最后两层之外的所有层都由鉴别器共享
        # Number of nodes in the last layer is reduced by half. It gives better results. 最后一层中的节点数减少了一半。 它提供了更好的结果。
    ...
    def build_model(self):
        ...
        # discrete code : categorical 离散代码，分类
        ...
        # continuous code : gaussian 连续代码，高斯
    ...
    def train(self):
        ...
        self.test_labels = self.data_y[0:self.batch_size] #分类标签数据
    ...
    def visualize_results(self, epoch):
        tot_num_samples = min(self.sample_num, self.batch_size)
        # np.sqrt返回非负平方根 np.floor 返回不大于输入参数的最大整数
        image_frame_dim = int(np.floor(np.sqrt(tot_num_samples)))

        """ random noise, random discrete code, fixed continuous code """
        # choice方法会随机生成batch_size大小的随机数组，数组中每个元素的大小为0 ~ len_discrete_code这个范围
        y = np.random.choice(self.len_discrete_code, self.batch_size)
        # 生成 (64,12) 形状的矩阵
        y_one_hot = np.zeros((self.batch_size, self.y_dim))
        # 构造成独热向量，y中对应的数，在y_one_hot对应的位置赋值为1
        ...
        for l in range(self.len_discrete_code):
            ...
            if l == 0:
                ...
                # 每个维度对应一种图片，将生成的图片合成为一个大的all_sample，然后再通过save方法保存
        ...
        # np.meshgrid从坐标向量返回坐标矩阵。 https://www.jianshu.com/p/99ade8578844
        xv, yv = np.meshgrid(c1, c2)
        xv = xv[:image_frame_dim,:image_frame_dim]
        yv = yv[:image_frame_dim, :image_frame_dim]
        # numpy flatten() 返回一个折叠成一维的数组，其实就是将多维数组每一维首尾链接成一维数组
        c1 = xv.flatten()
        c2 = yv.flatten()
