...
class vaegan(object):
    ...
    def __init__(self, batch_size, max_iters, repeat, model_path, data_ob, latent_dim, sample_path, log_dir, learnrate_init):
        ...
        self.latent_dim = latent_dim #隐变量的维度
        ...
        self.ep = tf.random_normal(shape=[self.batch_size, self.latent_dim]) #从正态分布中获取数据
    ...
    def build_model_vaegan(self):
        ...
        # 判别器判别生成图像(直接正态分布中获得的噪音)
        _, self.G_pro_logits = self.discriminate(self.x_p, True)

        #KL loss --> p(z)真实分布与q(z|x)变分分布 距离最小化
        ...
        #For encode
        self.encode_loss = self.kl_loss/(self.latent_dim*self.batch_size) - self.LL_loss / (4 * 4 * 256) # 4 * 4 *256 是图片对应的不同维度
    ...
    def test(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            real_images, sample_images, fake_images = sess.run([self.images, self.x_tilde, self.x_p], feed_dict={self.images: next_x_images})
            save_images(sample_images[0:self.batch_size], [self.batch_size/8, 8], '{}/train_{:02d}_{:04d}_con.png'.format(self.sample_path, 0, 0))
            save_images(real_images[0:self.batch_size], [self.batch_size/8, 8], '{}/train_{:02d}_{:04d}_real.png'.format(self.sample_path, 0, 0))
            save_images(fake_images[0:self.batch_size], [self.batch_size / 8, 8],
                        '{}/train_{:02d}_{:04d}_fake.png'.format(self.sample_path, 0, 0))
    ...
    # VAE中对应的KL散度的损失，本质是最小化变分分布q(z|x)与p(z)
