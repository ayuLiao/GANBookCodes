...
def create_critic_loss(cumulative_rewards, estimated_values, present):
  ...
  """Critic对生成值评估的损失，经过训练Critic会越评估越准确"""
...
def calculate_reinforce_objective(hparams,
  ...
  计算REINFORCE目标。 REINFORCE的目标应仅限于缺失的元素。
   具体而言，最终的Generator奖励应该基于Discriminator对缺失令牌的预测。
   日志概率应仅针对缺失的令牌，并且应仅基于缺失的令牌计算基线。
  ...
  对于此模型，我们优化奖励是Discriminator分配给分配的*条件*概率的对数。 具体来说，对于鉴别过去情境的输出真实概率的鉴别器D，
  ...
  并且生成器G的策略是在给定过去上下文的情况下采取行动x2的对数概率。
