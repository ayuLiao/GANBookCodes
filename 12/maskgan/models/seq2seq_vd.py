...
def transform_input_with_is_missing_token(inputs, targets_present):
  ...
  # Input present mask.
  # 输入当前掩码 --> condition True/False 的矩阵
...
def gen_encoder(hparams, inputs, targets_present, is_training, reuse=None):
  ...
  with tf.variable_scope('encoder', reuse=reuse):
    ...
    if is_training and hparams.gen_vd_keep_prob < 1:
      ...
      def attn_cell():
        # 在 LSTM Cell 外加一层 dropout增加随机性，即正则化
    ...
    # tf.contrib.rnn.MultiRNNCell 获得多层Cell的叠加
    ...
    with tf.variable_scope('rnn') as scope:
      ...
      # 将 embedding 变量分为两部分，一部分通过 embedding去获取，另一部分通过missing_embedding去获取
      # 这样就可以只让一部分变量去加载PTB数据训练出的权重了
  ...
  with tf.variable_scope('decoder', reuse=reuse):
    ...
    with tf.variable_scope('rnn'):
      ...
      if not FLAGS.seq2seq_share_embedding:
        embedding = tf.get_variable('embedding',
                                    [FLAGS.vocab_size, hparams.gen_rnn_size])
      # 通过 word embedding matrix 作为 Softmax_W的matrix
      ...
      for t in xrange(FLAGS.sequence_length):
        ...
        # 如果真实的入手数据存在，则读取真实的输入数据，如果不存在，则读入前面生成的数据
        else:
          ...
          if is_validating or FLAGS.gen_training_strategy == 'cross_entropy':
            ...
            # 序列数据中缺失的部分使用fake data，其余部分使用real data
        ...
        if FLAGS.gen_training_strategy != 'cross_entropy':
          ...
          # 如果本来就有真实的编码，则使用Real Token，如果是空缺的，才使用Fake Token
          real = targets[:, t]
...
def discriminator(hparams,
  ...
  if FLAGS.dis_share_embedding:
    # 判别器与生成器要共享模型的编码参数，就需要要求其具有的维度
    assert hparams.dis_rnn_size == hparams.gen_rnn_size, (
        '如果共享生成器与判别器的embedding, 它们必须具有相同的维度')
